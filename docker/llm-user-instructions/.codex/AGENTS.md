...
User-level defaults for Codex on this machine.
...
**On session start:** Call `agentpalace_wake_up(agent_name: "codex")` to orient yourself — this loads identity, palace status, recent changes, and diary entries in one call.
...
**On session end:** Write a diary entry with `agentpalace_diary_write(agent_name: "codex", ...)` summarising what happened and what matters.
