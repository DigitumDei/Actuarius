...
**On session start:** Call `agentpalace_wake_up(agent_name: "claude")` to orient yourself — this loads identity, palace status, recent changes, and diary entries in one call.
...
**On session end:** Write a diary entry with `agentpalace_diary_write(agent_name: "claude", ...)` summarising what happened and what matters.
