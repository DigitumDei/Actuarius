...
Known upstream limitation: each scheduling yield remains visible as `input_required` followed by unowned `pending`. Another generic AgentPalace consumer could claim it even though Actuarius retains the checkpoint locally. Consumers must respect the `actuarius` executor contract; native executor affinity requires an upstream change. The upstream change is tracked in [AgentPalace #153](https://github.com/DigitumDei/agentpalace/issues/153); [the issue text](agentpalace-yield-issue-draft.md) is also retained locally. No native yield transition is claimed to exist.
