...
The dispatcher, SQLite queue/work registry, AgentPalace adapter, and Git provisioning live in `src/services/coordination/`. `src/discord/coordinationBridge.ts` adapts Discord input and execution services. With coordination active, `providerGate.ts` serializes provider launches process-wide, including legacy paths. With coordination disabled it preserves legacy concurrency; waiting legacy requests receive activity heartbeats. Reviewers run sequentially. Keep one bot process on the host: the provider gate is process-local, not a distributed semaphore.

Enable with `COORDINATION_ENABLED=true`, `MEMPALACE_ENABLED=true`, and `COORDINATION_CHANNEL_ID`. Both the database and repository volume must persist. If AgentPalace is unavailable at startup, the bot starts with legacy commands and /tasks explains the disabled dispatcher. Restart after restoring AgentPalace to activate coordination; subsequent outages retain queued registrations and saved status. Polling runs every five seconds. Each configured source maintains its own discovery cursor; completed scans restart so older tasks changing state are reconsidered. Exact task/message operations use AgentPalace's authority routing. Keep dependencies for one workflow at one authority in v1.

Supported actions are `ask`, `implement`, `plan`, `plan-oc`, `review`, `revise`, `pr`, `handoff`, `report`, and `workflow`. A workflow has no workspace and delivers a report. `pr` and `handoff` require `draft_pr`. `revise` runs one focused implementation pass against the supplied findings on the retained branch; it does not invoke the planner or expand into a new task list, even with `iterative:true`. Existing requirements constrain the repair, completed work is retained, and normal acceptance verification, draft CI and final review still apply. Report-only revisions inspect without editing. Use `plan` explicitly when a new plan is needed. Plans with report delivery stop after planning; other plans implement, verify, and persist each continuation. `iterative:false` selects a single implementation step followed by acceptance verification. `/plan-oc` uses the managed OpenCode model choices with supervisor-controlled sequential stages. It does not launch the previous native delegation session. Plain implementation also gets a separate acceptance-verification step. Draft PR delivery uses the configured adversarial review; `/pr` requires a completed review of the unchanged branch.
...
The supervisor uses a persisted worker identity, revision-checked claims, 120-second leases renewed every 30 seconds, and aborts on lease loss. AgentPalace 0.1.48 provides an atomic running-to-pending yield with a reason, releasing the lease while retaining executor affinity without emitting a human-input wait. A workspace remains owned by an unfinished execution across continuation steps; other workspaces can use the LLM slot meanwhile. A failed readiness check does not stall unrelated repositories.
...
On an interrupted invocation, retain files and require inspection plus an explicit correction or `/revise`. Operator revision resumes a waiting task under the same native ID and moves its new attempt to Discord priority, so successful dependents can eventually unblock. Changed specs discard the execution checkpoint and revalidate against the retained worktree; an identical replacement preserves the interrupted stage. `/revise` with `resume`, `retry`, `continue`, or `resume the review` resumes the retained stage without adding recovery requirements or replanning. A correlated plain-text `resume`/`carry on` reply to an interrupted or handed-off task also preserves its checkpoint. A substantive correction still replaces intent and invalidates incompatible stage output. Completed provider output is saved before result publication; publication and notification failures retry delivery without running the implementation again. A crash before that output checkpoint requires inspection because external side effects may already have happened.

Generic cleanup skips open registered workspaces. Explicit `/delete` refuses unresolved tasks, dirty files, and commits not integrated into the target; a successfully closed work ID cannot silently allocate a fresh branch. Squash-merged workspaces can be deleted after matching the retained HEAD to an exact merged PR and checking its merge commit is in the integration target. Failed terminal tasks do not block this safe-integration check. No automatic retention deletion, merge, release, or cross-host execution is enabled.
...
Infrastructure errors during validation remain retryable and include available stderr diagnostics. After a successful fetch, missing base refs or integration branches are returned as validation questions. Renewal failures retry within the confirmed lease; a deadline watchdog stops work before lease expiry even if the renewal request hangs. Unmet dependencies back off from one to ten minutes; completion of a locally tracked prerequisite clears its dependent wait immediately on the next tick, and gate fetches share a one-minute repository cache. Discovery remembers observed revisions so unrelated unchanged tasks are not fetched again.

Discord `/ask` returns its answer directly after validation and execution, without an extra acceptance-verification stage; it changes files only when explicitly requested. Issue summaries fetch issue bodies at execution, outside the bounded requirements JSON. Standalone issue/report commands use the existing checkout and repository channel without creating a worktree or branch. Attachments download at intake into durable task-specific storage, then copy into the git-excluded worktree attachment directory before execution so image and clipped-text prompt paths resolve inside the provider workspace. Archiving queues retryable removal of the intake copy; metadata is cleared only after cleanup succeeds.

Question aliases are correlated to the validation attempt, so either the original or escalated question can be answered while open. Answers and thread follow-ups require the requester or server-manager permission; stale questions and invalid replacements receive explicit feedback. Rejected agent corrections receive a reason. Native registration includes a stable local-entry marker for recovery, and Discord thread creation is single-flight per work ID. Legacy status/cancel commands fall through without adopting a workspace.

Entry/event/native-task lookups use SQLite indexes. Request synchronization groups entries once, and retries no longer change immutable creation order. Terminal entries in closed work archive after 30 days into compressed records retaining exact-ID lookup and event deduplication. Task-list views expire after seven days; archived delivery markers are pruned. Small question and message correlation tombstones remain so old replies cannot accidentally become new work. Open-work audit history is retained. Provisioning, shared checkout updates, and cleanup share the repository lock.

AgentPalace #153 is implemented in the pinned 0.1.48 candidate. Validation and execution continuations use native yield; affinity survives lease expiry and requires the persisted Actuarius worker identity to resume. Foreign-affinity tasks are skipped. Preserve the coordination database and checkpoints when upgrading; an unavailable executor requires state restoration or cancellation and fresh work. Explicit checkpoint handoff is outside Actuarius v1. Older AgentPalace servers are unsupported for automatic coordination.

Validator invocation or output-parsing failures have a separate persistent limit of three consecutive failures per validation attempt. A valid verdict resets the counter; reaching the limit requests input through the durable native transition. Infrastructure checks retain their independent retry behavior. Cancellation acknowledges ephemerally before AgentPalace calls, retries revision conflicts up to three times, and distinguishes a confirmed cancellation from already-terminal or unconfirmed registration states. Legacy-thread authorization uses saved request data before acknowledgement; adoption runs afterward. Text follow-ups receive a queuing acknowledgement before attachment downloads and update that message with the outcome.

Recovery target selection: /revise prioritizes the unfinished task holding workspace ownership over newer follow-ups, including cancelled tasks, and retains the original requirements and acceptance criteria. /review and /pr refuse to enqueue while an unfinished task retains the workspace and explain how to recover it. Intake errors are returned in the deferred Discord reply.


### Human clarification and text follow-up recovery

For Discord-owned tasks (including native tasks taken over with `/revise`), a human answer queues a brief reconciliation before validation. The reconciler updates requirements, acceptance criteria, action, and deliverable together; the latest explicit clarification supersedes older conflicting criteria. Workspace identity, repository setup, and dependency gates cannot be changed by this step. A complete structured correction bypasses reconciliation. Native tasks still owned by their submitting agent continue to send human answers back to that agent.

A plain text follow-up to a task awaiting input or interrupted execution resumes that task. The retained workspace owner takes priority over newer orphan follow-ups; an explicit reply selects its referenced task. Ambiguous work without an owner still requires a task-specific reply. Attachment submissions keep their existing intake path. Running tasks continue to accept separately queued follow-ups with dependencies.

Clarification and validation receive the registered workspace and related task briefs, prioritizing the workspace owner. Follow-up question execution receives the same context. Validators treat retrieval of referenced AgentPalace requirements and review of the existing branch as actionable operations; they ask only about missing information that actually prevents execution. Reconciliation uses the planner runner and the same serialized, leased queue as validation, and failed reconciliation retains its pending clarification for retry.


Root-channel requests remain independent workspaces, with no dependency added merely because another task in the repository is stalled. Context lookup resolves the repository from the incoming specification before workspace registration, then includes active tasks in other workspaces of that repository. Explicit task references and workspace owners are prioritized; each candidate includes its task ID, work ID, branch, status and brief. Other repositories are excluded, the candidate list is bounded with an omission count, and ambiguous candidates are not automatically treated as the newest task. Executors refresh the authoritative AgentPalace record before applying an authorized task update.


Review hardening: serialized related-task context is capped at 32 KiB, including JSON escaping. Oversized briefs, reasons and results are marked as truncated; lower-priority tasks are omitted with an accurate omission count. Failed tasks retaining workspace ownership remain discoverable across repository workspaces. Ordinary queued follow-ups preserve the selected owner as their predecessor and use workspace authorization; only recovery of an existing task additionally checks its sender authorization.

The reconciler cannot independently escalate a report or workspace-change task to `draft_pr`. Direct authorized human replies such as `Create a draft PR`, `Draft PR please`, or `Draft PR approved, change acceptance criteria` create a separate persisted approval record. Existing authorized draft delivery is preserved, while unrelated, negated or quoted publication text does not grant approval. Unrecognized wording that would escalate publication returns an explicit approval question. Complete structured corrections remain authoritative; successful reconciliation or replacement clears the pending approval record.


## Agent workflow recovery and early draft CI

These fixes follow the 2026-09-17–18 demo-hub failure register and issue #228. They require deploying this code; they do not change production tasks by editing the repository.

### Lease and stop diagnostics

A renewal timeout can mean the server committed a renewal but its response was lost. Renewal failures now trigger a bounded authoritative task read. The worker accepts a refreshed revision only when the task is still running, owned by this worker, has compatible executor affinity, and has an unexpired lease outside the 15-second safety margin. A refreshed revision permits one immediate retry. A failed read never extends the last confirmed deadline. Ownership loss or expiry aborts the provider; unavailable renewals still hit the independent safety deadline.

Subprocess and provider errors preserve the stop kind (`operator`, `shutdown`, `lease_loss`, generic `cancelled`, or `quota`), actual reason, provider/session identifiers, stage, and partial output. Idle and total timeouts retain their existing distinct fields. The durable `last-stop:<entry ID>` metadata records bounded partial output and whether an execution checkpoint survived. Review-stage errors include the reviewer/round or analyzer/judge/summarizer stage. A provider quota stop pauses recovery rather than becoming a code finding or successful review. Historical cancellations without logs remain undiagnosed.

### Draft publication and CI stages

An explicitly authorized `draft_pr` implementation publishes its first meaningful committed checkpoint before final review. Iterative plans publish after each implementation substep, so CI can inform verification while later steps remain unfinished. Implementers still cannot push or open PRs themselves. `workspace_changes` and `report` never enter publication stages. Local toolchain restrictions, including the deployed Rust stubs, remain applicable.

The internal continuation stages are `publish-draft`, `draft-ci`, `ci-fix`, and `deliver`. Before network publication, the implementation report and intended continuation are saved. A failed push or PR operation resumes publication rather than rerunning implementation. The retained work record stores the draft URL, published SHA, CI evidence and local review state. Retries reuse the branch and existing open draft.

CI collection paginates check runs and commit statuses for the exact published SHA. It selects the latest rerun per check name/application and latest status per context, and includes failed Actions job logs where available. An old commit's green checks cannot approve a newer checkpoint. Code failures feed back to implementation; each correction must produce a new commit. Corrections are bounded to three attempts. Pending or unavailable checks yield the execution slot and poll once a minute, with a 20-poll bound before operator input. An explicit resume starts a fresh polling window without resetting the corrective-attempt count. Cancellation, timeout, startup failures, missing checks and unavailable APIs do not become a pass. The GitHub App needs read access to Checks and commit statuses; Actions read access supplies failure logs.

Final acceptance requires review approval, unchanged local HEAD and source files, an unchanged open draft head, and passing reported CI for that same SHA. The PR remains draft; merging, releasing and deploying remain external decisions. Substep verifiers evaluate their own implementation step and receive publication/CI evidence separately; later steps and merge readiness are not substep blockers.

### Active review scope and durable recovery

The coordination review runner receives the active task's full effective spec directly. It no longer concatenates every task description sharing a work ID. The analyzer, reviewers, peer critiques, judge and summarizer all receive the authoritative requirements, acceptance criteria, deferred work and exact local/published/base SHA evidence. Parent and sibling scope cannot silently replace this input. Actual scoped defects still require correction.

Completed stage outputs are cached in the coordination database by code SHA, diff, active scope, publication evidence, prompt, stage, reviewer configuration and model. A provider interruption resumes unfinished calls, reusing completed outputs only when those inputs still match. Read-only snapshot checks run before and after stages; output produced against a changed workspace is not cached. Changed criteria, code, baseline or model invalidate reuse. Interrupted review is never approval. Cache entries are removed when closed tasks are archived.

Brief validation treats native dependencies as scheduler ordering. An unfinished predecessor is normal waiting, not contradictory intent, and validators may not invent extra predecessors. Native dependency completion and explicit merged/release/stacked gates remain scheduler checks.

### Explicit checkpoint handoff and external completion

Use `action: "handoff"` with `deliverable: "draft_pr"` for an authorized cleanup/publication checkpoint that another operator will finish. It implements the requested cleanup, commits and publishes the checkpoint, then enters `input_required` with its publication and CI continuation retained. It does not start another adversarial review or declare the unreviewed work completed. An authorized clarification on an existing draft task can select this action; generic requests to finish do not imply a handoff or new publication permission. This action does not create a remote Codex task or send a message to another service.

External open-draft updates are accepted only when the retained branch can fast-forward, or the remote is an ancestor of retained local work. Divergence, dirty source files and incompatible PR state require inspection. Normal publication never force-pushes. Git and GitHub publication subprocesses receive the execution cancellation signal; lease loss stops the request and preserves its diagnostics. PR bodies use temporary files to preserve large multiline reports. Changes during review still invalidate acceptance.

Periodic reconciliation only observes a matching external merge and queues `reconcile-publication`. Integration and native completion run in the worker slot under a fresh task lease. The task spec, registered branch/base and PR must still match; the published checkpoint and retained local head must be ancestors of the confirmed external PR head. Its merge commit must exist in the integration target. The worker preserves external commits, integrates current target history without resetting the branch, and publishes the native result without trying to republish the merged PR. Conflicts abort the integration and require inspection. Before implementation, a later slice sharing the work ID also reconciles its completed predecessor's matching external merge under its own execution lease. It then uses the integrated retained branch and opens the next draft. Dirty or divergent retained work blocks that continuation before implementation starts.

Legacy tasks without a durable publication record can be recovered only when the native task retains workspace ownership and a recorded output checkpoint. The exact registered branch/base identifies the older PR; leased reconciliation then proves that the task checkpoint and retained head are included in the external merge. Missing ownership, SHA evidence or compatible history requires operator inspection. Absence of new metadata is not evidence that an older PR was never published. These changes do not retroactively diagnose why the historical task #160 branch changed or automatically alter the already-merged AgentPalace PR #168.

Regression coverage includes response-lost renewal, real ownership loss and unavailable refresh/expiry; the eight-task branched dependency graph; early plan checkpoints; failing CI followed by a corrective commit and exact-SHA success; missing/cancelled CI; stage cache reuse/invalidation; changed review heads; paused handoffs; and externally merged retained work. Live Discord/provider/GitHub validation remains a deployment smoke test.
