...
Known upstream limitation: each scheduling yield remains visible as `input_required` followed by unowned `pending`. Another generic AgentPalace consumer could claim it even though Actuarius retains the checkpoint locally. Consumers must respect the `actuarius` executor contract; native executor affinity requires an upstream change. The proposed issue is saved in [the local draft](agentpalace-yield-issue-draft.md), pending approval to publish in the separate repository. No native yield transition is claimed to exist.
