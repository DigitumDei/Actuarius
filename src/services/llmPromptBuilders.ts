...
export function buildOpencodePlanWorkflowPrompt(input: {
  ...
  return [
    ...
    `You must plan first, then invoke the foreground \`${input.implementerAgent}\` subagent one or more times to perform planner-directed implementation steps.`,
    "Wait for each task to complete, inspect its result and the resulting diff, and issue targeted follow-up tasks when needed before returning the required final report.",
    "You, not the implementer, decide the plan, acceptance criteria, and whether further implementation or corrections are required.",
    "Do not edit files directly, do not use a background task, and do not return before the implementation task finishes.",
    "Actuarius verifies completed subagent task events and will fail the request if delegation is skipped.",
    "",
    "Original request:",
    input.requestPrompt
  ].join("\n");
