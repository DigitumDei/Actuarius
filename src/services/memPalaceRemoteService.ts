...
export class MemPalaceRemoteService {
  ...
  /**
   * Seed the palace identity file agents receive from mempalace_wake_up.
   * Only writes when the file is absent so operator edits persist; failure is
   * non-fatal — agents just wake up without an identity, as before.
   */
  private async ensureIdentityFile(): Promise<void> {
    const identityPath = join(this.homeDir, ".mempalace", "identity.txt");
    if (await pathExists(identityPath)) return;
    try {
      await mkdir(dirname(identityPath), { recursive: true });
      await writeFile(identityPath, IDENTITY_TEMPLATE, "utf8");
      this.logger.info({ identityPath }, "Seeded MemPalace identity file");
    } catch (error) {
      this.logger.warn({ error, identityPath }, "Could not seed MemPalace identity file");
    }
  }
